...
```{r}
tterreno_wos <- tterreno_final
```


Para llevar a cabo un análisis cluster se procede a escalar las variables para que se encuentren en la misma magnitud de medida.

```{r escalado, echo = FALSE}
tterreno_final <- scale(tterreno_final)
tterreno_final <- as.data.frame(tterreno_final)
```
...
```{r, echo = FALSE}
q.dist <- get_dist(tterreno_final, method = "pearson") 
```
...
```{r, echo = FALSE, results = 'hide'}
tterreno_final.cor <- cor(t(tterreno_final), method = "pearson") 
round(tterreno_final.cor[1:nrow(tterreno_final), 1:nrow(tterreno_final)], 2)
...
```{r, echo = FALSE}
dist.eucl <- daisy(tterreno_final, metric = c("euclidean"), stand = FALSE)
```



```{r, echo = FALSE}
corrplot(as.matrix(dist.eucl), is.corr = FALSE, method = "color", type="lower", diag=F, order="hclust", tl.cex=0.4, tl.col="blue")
...
```{r, echo = FALSE}
fviz_nbclust(x = tterreno_final, FUNcluster = kmeans, method = "wss", k.max = 15, 
             diss = get_dist(tterreno_final, method = "euclidean"), nstart = 50)
```


```{r, echo = FALSE}
tterreno.eclust <- eclust(tterreno_final, FUNcluster = "kmeans", stand=TRUE, hc_metric="euclidean", nstart=25, k = 7)
```

```{r, echo = FALSE}
tterreno.eclust$size
```



```{r, echo = FALSE}
fviz_silhouette(tterreno.eclust)
```


```{r, echo = FALSE}
tterreno.eclust$cluster
```


```{r, echo = FALSE}
tterreno_last <- data.frame(tterreno_wos, cluster = tterreno.eclust$cluster)
tterreno_last
```


```{r, echo = FALSE}
medidas_garajes <- tterreno_last %>%
  group_by(cluster) %>%
  summarize(mean_precio = round(mean(precio),1),
            mean_cilindrada = round(mean(cilindrada),1),
            mean_potencia = round(mean(potencia),1),
            mean_revoluciones = round(mean(revoluciones),1),
            mean_peso = round(mean(peso),1),
            mean_plazas = round(mean(plazas),0),
            mean_consumo120 = round(mean(consumo_120),1),
            mean_consumourb = round(mean(consumo_urbano),1),
            mean_vel_max = round(mean(velocidad_maxima),1))
medidas_garajes
```
