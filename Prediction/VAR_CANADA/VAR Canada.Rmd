...
El objetivo de este informe es poder predecir cómo evolucionarán el Producto Interior Bruto (GDP) y la inflación en Canadá en los próximos tres años a partir del segundo trimestre de 2019.

Se debe especificar que para obtener la inflación se han tenido que obtener previamente los datos referentes a los índices de los precios de consumo. Tras recabar dichos datos, se ha realizado una operación consistente en multiplicar por 100 los índices de los precios de consumo dividido entre dichos índices a lo largo de los 4 trimestres previos, a lo cual se le resta 1. De esta forma obtenemos la inflación como un porcentaje, y se puede ver fácilmente su evolución a lo largo del tiempo.

Los datos sobre el GDP y la inflación se han obtenido de las siguientes referencias:

1. Growth: Constant Price Gross Domestic Product in Canada https://fred.stlouisfed.org/series/CANGDPRQPSMEI

2. CPI: Consumer Price Index: All items: Total: Total for Canada https://fred.stlouisfed.org/series/CANCPALTT01IXOBSAQ


```{r data series, echo = FALSE, message = FALSE, warning = FALSE}
series = c('CANGDPRQPSMEI', 'CANCPALTT01IXOBSAQ') # FRED codes for US GDP growth and CPI
...
Data <- inner_join(tibble(Date = time(Growth), Growth = coredata(Growth)), 
                   tibble(Date = time(CPI), CPI = coredata(CPI)), 
                   by = c("Date")) %>% 
...
A continuación se muestra la evolución del GDP y la inflación hasta el segundo trimestre de 2019. También se puede ver la relación que guardan ambos al mostrarse de forma conjunta.
...
pivot_longer(Data, cols = -Date, names_to = "Variables", values_to = "Values") %>% 
  ...
  labs(title = "Canadian GDP growth and inflation",
       x = "", y = "",
...
# Predicción de los 12 siguientes trimestres (3 años)
...
ggplot(forecast_data) + 
  ...
  labs(title="Forecasts of GDP growth and inflation", 
       subtitle=paste("Quarterly data, annual rates of change, VAR with", m, "lags"), 
       caption=paste("Source: FRED series", paste(series, collapse=", ")),
       x="", y="")
```


Se puede observar en el gráfico la predicción de la evolución del GDP y de la inflación desde el segundo trimestre de 2019 hasta el segundo trimestre del año 2022.

Apreciamos que ambas predicciones se mantienen bastante estables a lo largo de los 3 años, siempre teniendo en cuenta las distintas bandas en función del intervalo de confianza que se selecciones. A pesar de ello, se ve que la mayoría de las predicciones, incluso las más negativas, se encuentran mayoritariamente en el rango máximo de la evolución histórica previa de ambas series, por lo que no parece probable que puedan darse unos valores excesivamente altos o bajos en los años que se pretende predecir.

Para el caso del GDP, parece que en los primeros trimestres se producirá un aumento del mismo, para terminar estabilizándose más adelante, sin perder la subida inicial. Mientras, en el caso de la inflación, se produce el efecto contrario, descendiendo la misma para estabilizarse más adelante y recuperar parte del valor perdido.
