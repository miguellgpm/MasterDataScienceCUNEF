...
# FECHA: 02/10/2019
# RESPUESTA A DIFERENTES CUESTIONES SOBRE BASE DE DATOS DE NOTAS DE ALUMNOS
...
presentados_examen_final <- subset(calificaciones_ECO_2019, calificaciones_ECO_2019$`Ex. JUNIO-12P` != "NA")
View(presentados_examen_final)

media_alumnos_presentados_examen_final <- mean(presentados_examen_final$`Ex. JUNIO-12P`)
cat("La nota media del examen de junio es", round(media_alumnos_presentados_examen_final, 2)) # REDONDEADA A 2 DECIMALES

# B) Nombre de los que han aprobado el examen final a pesar de haber asistido menos de un 50% a las clases

aprobados_asistencia_inferior_mitad <- function(presentados_examen_final) {
  numero_alumnos <- length(presentados_examen_final$`NOMBRE ALUMNO`)
  lista_nombres <- c() # lista vacia para incluir casos
  for (i in 1:numero_alumnos) {
    if (presentados_examen_final$`Asistencia -1P`[i] < 0.5 & presentados_examen_final$`Ex. JUNIO-12P`[i] >= 6) {
      lista_nombres <- append(lista_nombres, presentados_examen_final$`NOMBRE ALUMNO`[i])
    }
  }
...
aprobados_asistencia_inferior_mitad(presentados_examen_final)
...
for (i in 1:nrow(calificaciones_ECO_2019)) {
  if (calificaciones_ECO_2019$CALIFICACION[i] == "SUSPENSO" | calificaciones_ECO_2019$Nota_FINAL[i] < 5) {
